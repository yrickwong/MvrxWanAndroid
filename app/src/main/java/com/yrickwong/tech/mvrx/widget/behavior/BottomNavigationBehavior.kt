package com.yrickwong.tech.mvrx.widget.behavior
...
class BottomNavigationBehavior(context: Context, attrs: AttributeSet) :
    ...
    override fun onNestedPreScroll(
        ...
        if (dy > 0) {// 上滑隐藏
            if (outAnimator == null) {
                outAnimator =
                    ObjectAnimator.ofFloat(child, "translationY", 0.0f, child.height.toFloat())
            ...
            if (inAnimator == null) {
                inAnimator =
                    ObjectAnimator.ofFloat(child, "translationY", child.height.toFloat(), 0.0f)
